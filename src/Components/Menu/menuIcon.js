...
const MenuIcon = (props) => {
    ...
    return (
        <Link className={`menu--item ${props.open === props.id ? 'active' : ''}` } to={props.rota} onClick={props.onClick}>
            <Icon className={`menu--icon ${props.open === props.id ? 'active' : ''}`} />
            <p className="menu--tab">{props.descricao}</p>
            {props.open === props.id ? activeIcon : null}
         </Link>
    )
}
